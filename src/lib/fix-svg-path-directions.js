/**
 * wrapper to fix path directions from stringified pathdata
 * parses and normalizes path before
 */
export function getFixedPathData(d, options) {
  options = {
    //defaults
    arcToCubic: false,
...
function fixPathDataDirections(pathData, toClockwise = false, sort = true) {
  ...
  for (let i = 0; i < polys.length; i++) {
    ...
    if ((!includedIn.length && cw && !toClockwise) || (!includedIn.length && !cw && toClockwise)) {
  ...
  if (sort) {
    ...
    pathDataArr = polys.map((poly) => {
      return pathDataArr[poly.index];
    });
  }
...
function parsePathDataNormalized(d, options = {}) {
    ...
    if (!hasRelative && !hasShorthands && !hasArcs && !hasQuadratics) {
      ...
      for (let i = 0; i < comChunks.length; i++) {
        ...
        if (isRel) {
          ...
          switch (typeRel) {
            case "a":
              com.values = [values[0], values[1], values[2], values[3], values[4], values[5] + offX, values[6] + offY];
              break;
            ...
            case "q":
            case "s":
              com.values = [values[0] + offX, values[1] + offY, values[2] + offX, values[3] + offY];
              break;
  ...
  for (let i = 0; i < pathData.length; i++) {
    ...
    pathData[i].values =
      pathData[i].values.length > 1
        ? pathData[i].values.map((val) => {
...
function arcToBezier(p0, values, splitSegments = 1) {
  ...
  const k = 0.551785;
  let a = ang2 === angle90 ? k : ang2 === -angle90 ? -k : (4 / 3) * Math.tan(ang2 / 4);
